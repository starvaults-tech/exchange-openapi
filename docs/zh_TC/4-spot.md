...
| ------------------------------------------ | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| symbol<mark style="color:red;">\*</mark>   | String  | 幣對名稱E.g.`BTC/USDT`                                                                                                                                                                                                                   |
| interval<mark style="color:red;">\*</mark> | String  | <p>k線圖區間, 可識別發送的值為：</p><p><code>1min</code>,<code>5min</code>,<code>15min</code>,<code>30min</code>,</p><p><code>60min</code>,<code>1day</code>,<code>1week</code>,<code>1month</code></p><p>（min=分鐘，h=小時,day=天，week=星期，month=月）</p> |
| startTime                                  | long    | 起始時間點                                                                                                                                                                                                                                |
| endTime                                    | long    | 截止時間點                                                                                                                                                                                                                                |
| timezone                                   | String  | 時區，eg:UTC-09                                                                                                                                                                                                                         |
| limit                                      | integer | 返回k線數量                                                                                                                                                                                                                               |
...
| type<mark style="color:red;">\*</mark>   | String  | <p>訂單類型,</p><p><code>LIMIT/MARKET/FOK/POST_ONLY/IOC/STOP</code></p> |
...
| type          | string | LIMIT                | 訂單類型，可用值:LIMIT,MARKET,STOP                                                                                 |
| transactTime  | string | `1273774892913`      | 訂單創建時間                                                                                                     |
...
若未提供 `symbol` 參數，API 將消耗顯著更高的頻率限制。

**Query Parameters**

| Name      | Type    | Description                                                                                                                                                                                          |
| --------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| symbol    | String  | 交易對名稱，例如：`BTC/USDT`                                                                                                                                                                                  |
| limit     | Integer | 預設 50；最大 100                                                                                                                                                                                         |
| startTime | Long    | <p>開始時間戳（毫秒）<br>・未傳入 startTime 與 endTime 時，預設回傳 7 天資料<br>・僅傳入 startTime 時，回傳 startTime 至 startTime+7天 的範圍<br>・僅傳入 endTime 時，回傳 endTime-7天 至 endTime 的範圍<br>・若兩者皆傳入，規則為 endTime - startTime ≤ 7 天</p> |
| endTime   | Long    | 結束時間戳（毫秒）                                                                                                                                                                                            |

* 若未傳入 symbol，將以陣列形式回傳所有交易對的成交紀錄。
* 僅支援查詢最近 6 個月內的資料。
...
| --------- | ------- | -------------------- | ------------------------------------------------------------ |
| id        | integer | `28457`              | 成交 ID                                                        |
| bidId     | long    | `150695552109032492` | 買單 ID                                                        |
| askId     | long    | `150695552109032492` | 賣單 ID                                                        |
| price     | integer | `4.01`               | 訂單價格                                                         |
| qty       | float   | `12`                 | 交易數量                                                         |
| time      | number  | `1499865549590`      | 創建時間                                                         |
| isBuyer   | bool    | `true`               | `true` = 買　`false` = 賣                                       |
| isMaker   | bool    | `false`              | <p><code>true</code> = 市價　</p><p><code>false</code> = 限價</p> |
| feeCoin   | string  | `ETH`                | 手續費幣種                                                        |
| fee       | number  | `0.001`              | 交易手續費幣                                                       |
| bidUserId | long    | `23334`              | 買方 UID                                                       |
| askUserId | long    | `44112`              | 賣方 UID                                                       |
| isSelf    | bool    | `true`               | 是否為自成交                                                       |
...
| Name     | Type   | 範例      | Description    |
| -------- | ------ | ------- | -------------- |
| `uid`    | string | 10001   | 用户id           |
| `asset`  | string | `USDT`  | 資產名稱           |
| `free`   | float  | 1000.30 | 可用金額           |
| `locked` | float  | 400     | 已鎖定金額（針對未完成訂單） |
...
| transferId  | String  | `1a9ec387-8b81-4789-a98e-bc6a606c8736` | 劃轉id |
| ----------- | ------- | -------------------------------------- | ---- |
...
| fromAccount | String  | <p>轉出賬戶</p><p>EXCHANGE 現貨賬戶</p><p>FUTURE 合約賬戶</p> |
...
| transferId  | String | `1a9ec387-8b81-4789-a9...` | 劃轉 ID                                                                 |
| ----------- | ------ | -------------------------- | --------------------------------------------------------------------- |
| fromAccount | String | `EXCHANGE`                 | 轉出帳戶                                                                  |
| toAccount   | String | `FUTURE`                   | 轉入帳戶                                                                  |
| coinSymbol  | String | `USDT`                     | 幣種                                                                    |
| createTime  | long   | `1742300000000`            | 創建時間戳                                                                 |
| amount      | String | `1`                        | 數量                                                                    |
| status      | String | `SUCCESS`                  | <p>狀態說明：</p><p>SUCCESS = 成功</p><p>PENDING = 劃轉中</p><p>FAILED = 失敗</p> |
